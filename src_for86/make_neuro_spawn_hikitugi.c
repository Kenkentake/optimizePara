...
int main(int argc, char **argv){
	...
	for(k=0;k<num_of_my_pop;++k){
	  for(i=0;i<dim_conMat;++i){
	    for(j=0;j<dim_conMat;++j){
	      pop_sendbuf_nrn_weight_adjust_dim[offset + num_of_cell_combination * k + dimension_per_one_nrnproc * i + j] = pop_rcvbuf_whole[gene_id] * conMat[i][j];
	      	      pop_sendbuf_nrn_weight[(dim_conMat * dim_conMat) * k + dim_conMat * i + j] = pop_rcvbuf_whole[gene_id] * conMat[i][j];
	      gene_id += conMat[i][j];
	    }
	  }
	}
